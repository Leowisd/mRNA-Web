...
class Attention(Layer):
    def __init__(self, hidden, da, r, att_weight=1, init='glorot_uniform', activation='tanh', W1_regularizer=None,
        ...
        print("att_weight in attention model",K.get_value(att_weight))
    ...
    def build(self, input_shape):
        ...
        #self.att_weight = self.add_weight(shape=(1),name = 'att_weight',initializer='ones',trainable =False)
    ...
    def call(self, H, mask=None):
        ...
        H_t = self.activation(K.dot(H, self.W1)) #H_t is [none,length,hidden]x [hidden,da] = [none,length,da]
        temp = K.permute_dimensions(K.dot(H_t, self.W2), (0, 2, 1))  # [?,r, length] temp = [none,length,da] x [da,r]= [none,length,r]
        
        #temp=K.square(temp)#make dis larger
        A = K.softmax(temp*self.att_weight)  # A    [none, r, n]
        M = K.batch_dot(A, H, axes=(2, 1))  # [none, r, hidden]
